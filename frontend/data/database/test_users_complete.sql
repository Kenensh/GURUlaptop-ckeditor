...
-- 5. 驗證插入結果
SELECT '=== Users 表格驗證結果 ===' as status;
SELECT COUNT(*) as total_users FROM public.users;
SELECT user_id, name, email, created_at FROM public.users ORDER BY user_id LIMIT 5;
SELECT '=== 序列狀態 ===' as status;
SELECT nextval('users_user_id_seq') as next_user_id;
SELECT setval('users_user_id_seq', 3); -- 重置回正確值
